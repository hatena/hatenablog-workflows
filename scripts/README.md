...
## test-resolve-working-directory.bash

`blogsync.yaml` から基準ディレクトリを判定する処理のテストです。リポジトリルート、サブディレクトリ、空白を含むディレクトリ、設定なし、未追跡、複数設定のケースを検証します。

```console
./scripts/test-resolve-working-directory.bash
```
